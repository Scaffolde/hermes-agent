...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            try:
                owner_profile = whatsapp_setup.resolve_whatsapp_gateway_profile(None)
                whatsapp_setup.prepare_whatsapp_pairing(
                    gateway_profile=owner_profile,
                    session_path=session_dir,
                )
            except Exception as exc:
                print(f"\n  ✗ Could not prepare WhatsApp pairing safely: {exc}")
                return
            ...
            if (get_env_value("WHATSAPP_ENABLED") or "").lower() != "true":
                try:
                    whatsapp_setup.activate_whatsapp_after_pairing(restart_gateway=False)
                except Exception as exc:
                    print(f"\n  ✗ Could not enable WhatsApp safely: {exc}")
                    return
    ...
    try:
        pair_env = whatsapp_setup.scrub_baileys_whatsapp_env(with_hermes_node_path())
        pair_env["WHATSAPP_MODE"] = wa_mode
        pair_env["WHATSAPP_DM_POLICY"] = "pairing"
        pair_timeout = max(
            30,
            int(os.environ.get("HERMES_WHATSAPP_PAIR_TIMEOUT_SECONDS", "300")),
        )
        subprocess.run(
            [
        ...
        print(
            "\n⚠ WhatsApp pairing timed out. WhatsApp remains disabled; "
            "run `hermes whatsapp` to try again."
        )
    except KeyboardInterrupt:
        pass
    ...
    if (session_dir / "creds.json").exists():
        ...
        try:
            whatsapp_setup.activate_whatsapp_after_pairing(restart_gateway=True)
        except Exception as exc:
            print(f"✗ WhatsApp paired, but could not enable it safely: {exc}")
            return
