...
def _spawn_whatsapp_pairing_process(session_path: Path, mode: str) -> subprocess.Popen:
    ...
    env = whatsapp_setup.scrub_baileys_whatsapp_env(with_hermes_node_path())
...
def _run_whatsapp_pairing(pairing_id: str, session_path: Path, mode: str) -> None:
    with _whatsapp_onboarding_lock:
        ...
        requested_profile = record.profile

    try:
        gateway_profile = whatsapp_setup.resolve_whatsapp_gateway_profile(requested_profile)
...
async def apply_whatsapp_onboarding(
    ...
    try:
        owner_profile = whatsapp_setup.resolve_whatsapp_gateway_profile(effective_profile)
        restart_started = whatsapp_setup.restart_gateway_if_running(profile=owner_profile)
        restart_result = {
            "restart_started": bool(restart_started),
            "restart_action": "gateway-restart" if restart_started else "none",
        }
    except Exception as exc:
        _log.exception("Failed to auto-restart gateway after WhatsApp onboarding")
        restart_result = {"restart_started": False, "restart_error": str(exc)}
    return {
        "ok": True,
        "platform": "whatsapp",
