...
def _resolve_external_process_command(command: str, provider_id: str) -> Optional[str]:
    resolved = shutil.which(command) if command else None
    if resolved:
        return resolved
    # GUI/launchd processes often omit ~/.local/bin from PATH; Antigravity's
    # installer commonly places `agy` there. Keep this deterministic fallback
    # internal to command resolution rather than mutating PATH globally.
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    resolved_command = _resolve_external_process_command(command, provider_id)
