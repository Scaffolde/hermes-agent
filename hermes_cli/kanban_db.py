...
def recompute_ready(
    ...
    done), *except* in deliberate stop states:

    1. The most recent block event was a worker/operator ``blocked`` event
       or a circuit-breaker ``gave_up`` event — those stay blocked until
       an explicit ``kanban_unblock`` clears the sticky state (#28712).
