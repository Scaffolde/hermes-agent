...
def _load_local_whisper_model(model_name: str, device: str = "auto", compute_type: str = "auto"):
    ...
    """
    # The host safety fallback only owns automatic selection. Explicit user
    # configuration is authoritative; otherwise a pinned CPU/float32 setup is
    # silently rewritten to CPU/int8 on Apple Silicon/Rosetta.
    force_cpu = (
        device == "auto"
        and compute_type == "auto"
        and _should_force_faster_whisper_cpu()
    )
    if force_cpu:
