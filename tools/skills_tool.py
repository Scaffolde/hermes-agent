...
def skill_view(
    ...
    try:
        ...
        # Strategy 4: search by frontmatter name across all dirs. skills_list
        # advertises the frontmatter name, so skill_view must accept the same
        # identifier even when the containing directory is categorized or
        # prefixed differently (for example gstack-browse/SKILL.md with
        # name: browse).  Record matches as candidates so upstream collision
        # detection still catches ambiguous local/external skill names.
        for search_dir in all_dirs:
            for found_skill_md in iter_skill_index_files(search_dir, "SKILL.md"):
                try:
