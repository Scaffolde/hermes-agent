...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        if hasattr(agent, "_resolved_stream_read_timeout"):
            _stream_model = str(api_kwargs.get("model") or getattr(agent, "model", "") or "").strip().lower()
            _stream_read_timeout = agent._resolved_stream_read_timeout(model=_stream_model)
            if _stream_read_timeout == _base_timeout:
                ...
                elif getattr(agent, "_is_nvidia_deepseek_v4_pro_route", lambda _model=None: False)(_stream_model):
                    logger.debug(
                        "NVIDIA DeepSeek V4 Pro route detected — stream read timeout raised to %.0fs",
                        _stream_read_timeout,
                    )
        elif _provider_timeout_cfg is not None:
        ...
        _is_nvidia_deepseek_v4_pro = getattr(
            agent,
            "_is_nvidia_deepseek_v4_pro_route",
