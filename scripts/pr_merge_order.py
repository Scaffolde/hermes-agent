...
Exit codes: 0 = a landable order exists, 1 = collisions need a human decision,
...
def compute_merge_order(
    ...
    for a_num, b_num in itertools.combinations(numbers, 2):
        ...
        # A PR with no verdict of its own cannot lend one to a pair: merging
        # onto a branch that does not merge into its own base answers nothing.
    ...
    blocked: set[int] = set(unverified)
    for pair in report.mutual_conflicts:
        blocked.add(pair.a)
        blocked.add(pair.b)
    # Anything stacked on a held PR cannot land either.
    for number in numbers:
        if any(anc in blocked for anc in stack_ancestors(number, stacked_on)):
            blocked.add(number)
...
class GitProbeSuite:
    ...
    def _land(self, pr: PullRequest) -> str:
        ...
        if not commit:
            raise RuntimeError(f"failed to synthesize landing commit for #{pr.number}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = compute_merge_order(
        probe_targets, suite.probe, suite.is_ancestor, preflight=suite.preflight
    )
    ...
    return 1 if (report.mutual_conflicts or report.diverged) else 0
