...
class TestSlackFirstClassUserTokenRouting:
    def test_ordinary_dm_stays_on_workspace_bot_client(self, adapter):
        user_client = AsyncMock()
        bot_client = AsyncMock()
        adapter._team_user_clients = {"T_ONE": user_client}
        adapter._team_clients = {"T_ONE": bot_client}
        adapter._channel_team = {"D_ONE": "T_ONE"}

        assert adapter._get_client("D_ONE") is bot_client

    @pytest.mark.asyncio
    async def test_addressed_thread_send_uses_user_client(self, adapter):
        user_client = AsyncMock()
        user_client.chat_postMessage = AsyncMock(return_value={"ts": "101.2"})
        bot_client = AsyncMock()
        adapter._team_user_clients = {"T_ONE": user_client}
        adapter._team_clients = {"T_ONE": bot_client}
        adapter._channel_team = {"C_ONE": "T_ONE"}
        adapter._user_addressed_threads = {("T_ONE", "C_ONE", "101.1")}

        result = await adapter.send(
            "C_ONE",
            "reply",
            metadata={"team_id": "T_ONE", "thread_id": "101.1"},
        )

        assert result.success
        user_client.chat_postMessage.assert_awaited_once()
        bot_client.chat_postMessage.assert_not_called()
        assert ("T_ONE", "101.2") in adapter._user_sent_message_ts

    @pytest.mark.asyncio
    async def test_addressed_thread_marker_is_workspace_scoped(self, adapter):
        user_client = AsyncMock()
        bot_client = AsyncMock()
        bot_client.chat_postMessage = AsyncMock(return_value={"ts": "202.2"})
        adapter._team_user_clients = {"T_TWO": user_client}
        adapter._team_clients = {"T_TWO": bot_client}
        adapter._user_addressed_threads = {("T_ONE", "C_SHARED", "202.1")}

        result = await adapter.send(
            "C_SHARED",
            "reply",
            metadata={"team_id": "T_TWO", "thread_id": "202.1"},
        )

        assert result.success
        bot_client.chat_postMessage.assert_awaited_once()
        user_client.chat_postMessage.assert_not_called()

    @pytest.mark.asyncio
    async def test_user_authored_message_edit_stays_on_workspace_user_client(
        self, adapter
    ):
        user_client = AsyncMock()
        user_client.chat_update = AsyncMock(return_value={"ok": True})
        bot_client = AsyncMock()
        adapter._team_user_clients = {"T_ONE": user_client}
        adapter._team_clients = {"T_ONE": bot_client}
        adapter._channel_team = {"C_ONE": "T_ONE"}
        adapter._user_sent_message_ts = {("T_ONE", "303.1")}

        result = await adapter.edit_message("C_ONE", "303.1", "updated")

        assert result.success
        user_client.chat_update.assert_awaited_once()
        bot_client.chat_update.assert_not_called()
...
class TestIncomingAudioHandling:
    ...
    async def test_voice_message_cached_with_correct_extension(self, adapter, tmp_path):
        ...
        async def _fake_download(url, ext, audio=False, team_id="", channel_id=""):
            captured["ext"] = ext
