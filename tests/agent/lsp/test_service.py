...
def test_reused_client_refreshes_last_used_and_survives_reap(mock_pyright):
    ...
    try:
        ...
        svc._loop.run(svc._sweep_idle_async(), timeout=5.0)
...
def test_reaper_survives_sweep_error(mock_pyright):
    ...
    try:
        ...
        calls = {"n": 0}
        real_reap = svc._sweep_idle_async

        async def _flaky_reap():
            calls["n"] += 1
            if calls["n"] == 1:
                raise RuntimeError("sweep sabotage")
            return await real_reap()

        svc._sweep_idle_async = _flaky_reap  # type: ignore[method-assign]
        ...
        assert svc.get_status()["clients"] == []
        assert svc._reaper is not None
        assert not svc._reaper.done()
    finally:
        svc.shutdown()
