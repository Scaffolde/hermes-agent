...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_antigravity_missing_cli_raises(self, monkeypatch):
        ...
        monkeypatch.setattr("hermes_cli.auth.Path.home", lambda: Path("/nonexistent-home"))
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_antigravity_uses_process_runtime(self, monkeypatch):
        monkeypatch.setattr("hermes_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setenv("HERMES_ANTIGRAVITY_CLI_ARGS", "--sandbox")

        from hermes_cli.runtime_provider import resolve_runtime_provider

        result = resolve_runtime_provider(requested="google-antigravity-cli")

        assert result["provider"] == "google-antigravity-cli"
        assert result["api_mode"] == "chat_completions"
        assert result["api_key"] == "google-antigravity-cli"
        assert result["base_url"] == "antigravity-cli://agy"
        assert result["command"] == "/usr/local/bin/agy"
