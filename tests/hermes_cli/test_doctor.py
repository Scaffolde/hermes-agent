...
def test_run_doctor_reports_kanban_as_runtime_gated_not_broken(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
    ...
    fake_model_tools = types.SimpleNamespace(
        ...
        TOOLSET_REQUIREMENTS={"terminal": {"name": "terminal"}},
    )
    monkeypatch.setitem(sys.modules, "model_tools", fake_model_tools)
    ...
    assert "kanban runtime-gated (available inside kanban worker tasks)" in out
    assert "kanban (availability check failed)" not in out
