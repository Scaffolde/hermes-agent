...
def test_show_status_all_still_redacts_api_keys(monkeypatch, capsys, tmp_path):
    raw_tavily = "tvly-dev-super-secret-status-token"
    raw_tinker = "tml-super-secret-status-token"
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    monkeypatch.setenv("TAVILY_API_KEY", raw_tavily)
    monkeypatch.setenv("TINKER_API_KEY", raw_tinker)

    show_status(SimpleNamespace(all=True, deep=False))

    output = capsys.readouterr().out
    assert "Tavily" in output
    assert "Tinker" in output
    assert raw_tavily not in output
    assert raw_tinker not in output
    assert "tvly...oken" in output
    assert "tml-...oken" in output
