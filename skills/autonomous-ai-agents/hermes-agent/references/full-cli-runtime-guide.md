# Hermes Agent Full CLI and Runtime Guide

This reference preserves the detailed guide that previously lived directly in `SKILL.md`. The skill entrypoint is intentionally compact so loading `hermes-agent` does not consume a large context budget.
